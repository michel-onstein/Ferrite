...
- **Vim mode: the full operator-motion keymap, plus `:` commands** — Vim mode was rebuilt as a keystroke/parser pipeline, replacing the `egui::Key` dispatcher that capped it. `egui::Key` has no variant for `$ ^ % * # ~ > < _`, so those were unreachable at any cost and `$` was faked with `End`; printable characters now come from text events instead, which also makes the keymap layout- and IME-independent. New: the whole motion set (`w W b B e E ge`, `0 ^ $ g_`, `gg G {count}G`, `f F t T ; ,`, `{ }`, `%`, `H M L`, `+ -`), every operator × motion pair (`d y c > < gu gU g~` and their doubled linewise forms), text objects (`ci"`, `dap`, `yi{`, `diw`, `daw`), registers (`"a`-`"z`, `"0`, `"-`, `"1`-`"9`, `"_`, `"+`), `.` repeat including text typed in the insert session, `u`/`Ctrl+R`, `x X r ~ J p P s S D C Y`, `gv`, and counts that multiply (`2d3w`). An ex command line adds `:w :wq :x :q :qa :e :{n} :{range}d :{range}y :s :%s :noh :set :{range}> :{range}<` — not vimscript. Motions are now grapheme-aware, so `h`/`l`/`x` no longer split emoji or combining marks. The cursor is a **block** in Normal/Visual/Command-Line mode and a thin bar in Insert mode, matching Vim, with the covered character repainted in a contrasting colour so it stays readable. Unrecognised keys pass through to the standard handler instead of being swallowed, so app shortcuts keep working in Normal mode; Vim's `Ctrl+D/U/F/B` are deliberately left unbound because Ferrite uses those for Delete Line, Find and Bold. See [`docs/VIM_MODE_DESIGN.md`](docs/VIM_MODE_DESIGN.md) and [`docs/technical/editor/vim-mode.md`](docs/technical/editor/vim-mode.md).
...
- **Vim mode: arrow keys now navigate** — In Vim Normal and Visual modes the arrow keys fell through to the catch-all match arm and were silently consumed, so only `hjkl` could move the cursor (reported on macOS; the cause was platform-independent). `←`/`↓`/`↑`/`→` now resolve to the same motions as `h`/`j`/`k`/`l` rather than being separately enumerated, so they honour repeat counts and extend the selection in Visual mode and cannot drift apart; `Home` acts as `0`, `End` as `$`, and `PageUp`/`PageDown` are page motions. In Insert mode all of them pass through to the standard grapheme-aware handler. The catch-all that caused this is gone — unrecognised keys now fall through to the standard handler instead of being swallowed, which closes the bug class rather than this one instance. See [`docs/technical/editor/vim-mode.md`](docs/technical/editor/vim-mode.md).
- **Test suite compiles again** — `cargo test` failed to build the test binary (42 errors), so no test in the repo could run. Test-only code referenced items that were never imported or had since moved: `TextBuffer` in `editor/ferrite/history.rs`, `CHECK`/`X` in `markdown/code_execution.rs`, the flowchart parser helpers and `expand_rect` in the Mermaid tests, and a missing `Task::to_markdown`. The suite runs again; five pre-existing failures are newly *visible* rather than new (Mermaid subgraph width float epsilon, bare-YouTube-URL paragraph parsing, two recovery-identity cases in `state.rs`, one Git untracked-file case), plus one flaky test (`markdown::cache::tests::block_height_cache_hit`, a shared-global-cache race between sibling tests). None are fixed here.
