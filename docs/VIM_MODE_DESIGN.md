...
- **The catch-all swallows keys.** Both Normal and Visual end in `_ => Consumed`. That is
  what made the arrow keys dead until v0.3.1, and it is a *latent* bug generator: every key
...
- `docs/technical/editor/vim-mode.md` lists `$`/`End` for end-of-line — only `End` works.
...
`vim.rs` currently carries 9 inline tests, all from the v0.3.1 arrow-key fix. The pipeline
